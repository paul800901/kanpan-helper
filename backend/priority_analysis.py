"""v30 排序分析與 orchestration 層：每日 priority 快照整理、分析報表與回填流程。

layer: analysis / orchestration
allowed dependencies: backend.priority_generation, backend.priority_contract_io, backend.priority_validation, shared base helpers
analysis / orchestration internal layer
...
from backend.config import get_taiwan_now
...
from backend.priority_contract_io import (
    _context_report_path,
    _load_context_report,
    _load_priority_contract_payload,
    _load_priority_contract_report,
    _priority_report_path,
    _save_priority_contract_report,
    _validate_priority_contract_payload,
)
from backend.priority_generation import (
    generate_priority_snapshot,
    PRIORITY_REPORT_VERSION,
    SORT_RULE_DESCRIPTION,
    to_num,
    _as_float,
    _round_number,
    _mean,
    ...
    _extract_return_pct,
    build_technical_map,
    compare_candidate_items,
    build_priority_explanation,
    get_advice_priority,
    get_zone_priority,
    get_zone_priority_label,
)
from backend.priority_validation import ScenarioCardValidationError
...
STEADY_V2_BLOCKER_CONDITION_DEFINITIONS = {
    # Internal compatibility only: facade delegates app/script snapshot generation
    # here; external callers should use backend.priority_facade.
    "ma20_v2": {
        "label": "MA20_v2 條件",
...
# =============================================================================
# V10.3 LAYER ARCHITECTURE: 第二頁分層架構
# =============================================================================
#
# 第二頁（Priority Validation）backend 三層架構：
#   1. Generation Layer: priority_generation.py
#      - 唯一可讀 legacy（cards, trace_catalog, stock_mapping_catalog）
#      - 產出正式 contract 欄位
#   2. Contract I/O Layer: priority_contract_io.py
#      - 負責第二頁正式 contract 的路徑、讀寫、payload 組裝與驗證前包裝
#   3. Analysis / Orchestration Layer: 本檔案 priority_analysis.py
#      - 整理 history / factor / strategy / steady_v2-v5 分析報表
#      - 串接 from_reports 入口、回填流程與報表落地
#      - 呼叫 generation / contract I/O / validation layer
#   4. Validation Layer: priority_validation.py
#      - 只驗證 contract 與第三頁 scenario card contract
#      - strict_mode guard 與正式 validator 都集中在此
#
# Frontend/UI 只讀正式 contract，不得直接依賴 legacy。
#
# Legacy Adapter 清單（僅限 Layer 1 使用）：
...
def ensure_context_report(date_str: str, base_dir: Optional[Path] = None, refresh: bool = False) -> Path:
    path = _context_report_path(date_str, base_dir)
...
def generate_priority_snapshot_from_files(
    ...
    context_report = _load_context_report(date_str, base_dir=base_dir, required=True)
    ...
    return _save_priority_contract_report(report, date_str=date_str, base_dir=base_dir)
...
def _collect_trading_interval_candidates(
    ...
    ordered_universe_dates = sorted(universe_lookup_by_date.keys())
    candidates: List[Dict[str, Any]] = []
    evaluated_days = 0

    for index, report in enumerate(ordered_reports):
        current_date = str(report.get("date") or "")
        next_date = str(report.get("next_report_date") or "")
        if _market_return(current_date, next_date, market_lookup) is None:
            continue

        previous_date = str(ordered_reports[index - 1].get("date") or "") if index > 0 else None
        current_universe_lookup = universe_lookup_by_date.get(current_date) or {}
        previous_universe_lookup = universe_lookup_by_date.get(previous_date or "") or {}

        if current_date in ordered_universe_dates:
            date_index = ordered_universe_dates.index(current_date)
            recent_dates = ordered_universe_dates[
                max(0, date_index - (MA20_V2_RECENT_BREAK_LOOKBACK - 1)):date_index + 1
            ]
        else:
            recent_dates = [current_date] if current_date else []
...
def _build_stability_summary(
    ...
    market_pairs = [
        ...
        for daily_return, market_return in zip(daily_returns, market_returns)
        if daily_return is not None and market_return is not None
    ]
    random_pairs = [
        (daily_return, random_return)
        for daily_return, random_return in zip(daily_returns, random_returns)
        if daily_return is not None and random_return is not None
    ]

    return {
        "evaluated_days": len(valid_returns),
        "avg_return": _mean(valid_returns),
        "return_std_dev": _std_dev(valid_returns),
        "positive_day_ratio": _round_number(
            (sum(1 for value in valid_returns if value > 0) / len(valid_returns)) * 100
        ) if valid_returns else None,
        "outperform_market_ratio": _round_number(
            (sum(1 for daily_return, market_return in market_pairs if daily_return > market_return) / len(market_pairs)) * 100
        ) if market_pairs else None,
        "outperform_random_ratio": _round_number(
            (sum(1 for daily_return, random_return in random_pairs if daily_return > random_return) / len(random_pairs)) * 100
        ) if random_pairs else None,
    }
...
# Internal compatibility only: report-loading and orchestration wrappers below
# remain for internal layers/tests. External callers should use the facade.
def load_priority_reports(base_dir: Optional[Path] = None) -> List[Dict[str, Any]]:
    ...
    for path in sorted(reports_dir.glob("*-priority.json")):
        ...
        if not DATE_PATTERN.match(date_str):
            continue
        reports.append(_load_priority_contract_report(date_str, base_dir=base_dir, required=True) or {})
    return reports
...
def backfill_priority_validation_reports(
    ...
    refreshed_context_path: Optional[str] = None
    if current_date:
        ensure_context_report(current_date, base_dir=base_dir, refresh=refresh_context)
        context_path, validation_payload = _load_priority_contract_payload(
            current_date,
            base_dir=base_dir,
        )
        refreshed_context_path = str(context_path)
        ensured_context_paths.append(refreshed_context_path)
        try:
            _validate_priority_contract_payload(validation_payload, context_path=context_path)
        except ScenarioCardValidationError as e:
            raise ScenarioCardValidationError(f"{refreshed_context_path} v9.1 contract 驗證失敗: {e}")
    ...
    return {
        ...
        "current_context_path": refreshed_context_path or (str(reports_dir / f"{current_date}-context.json") if current_date else None),
